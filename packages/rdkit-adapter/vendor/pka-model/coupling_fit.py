...
def load_network(path="site-pka-gnn.json"):
    ...
    # (value, spread), both of them: the fold weighs each rung by how much the ensemble disagreed
    # about it, so dropping the spread here would silently make every rung equally believable.
    return lambda mol, atom: predict_site(members, multiplier, mol, atom)
...
# How far a predicted rung is likely to be wrong, given the ensemble's own disagreement about it.
# Loaded from the artifact rather than typed in, and `protonation.ts` imports the SAME file: these two
# folds must weight identically or every polyprotic answer drifts between them with nothing failing to
# say so. Fitted by `edge_variance_fit.py`; regenerated by `run_all.sh`.
EDGE_VARIANCE = json.load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                            "edge-variance.json")))


def edge_weight(spread):
    """One rung's weight in the fold: the reciprocal of how wrong it is likely to be, squared."""
    s = EDGE_VARIANCE["unknownSpread"] if spread is None or not np.isfinite(spread) else float(spread)
    return 1.0 / (EDGE_VARIANCE["floor"] + EDGE_VARIANCE["perSpread"] * s * s)
...
def macroscopic(mol, sites, predict, W, dist, power=1.0, drop_tautomer_states=True):
    ...
    # Every rung the model can value, as an edge between two microstates. Collected first and solved
    # together -- see `macroscopicPka` in protonation.ts, which this mirrors edge for edge.
    edges = []
    for state in states:
        ...
        for i in range(n):
            ...
            if is_tautomer_state(lighter): continue
            acid = graph_of(state)
            ...
            try:
                got = predict(acid, atom_idx)
            except Exception:
                continue
            base, spread = got if isinstance(got, tuple) else (got, None)
            ...
            edges.append((state, lighter, float(base) + shift, edge_weight(spread)))

    # Only what the fully deprotonated reference can reach; anything else is left out of the partition
    # sum rather than guessed at.
    reference = tuple([0] * n)
    touching = defaultdict(list)
    for edge in edges:
        touching[edge[0]].append(edge)
        touching[edge[1]].append(edge)
    reachable, queue = {reference}, [reference]
    while queue:
        at = queue.pop(0)
        for acid, lighter, _v, _w in touching[at]:
            other = lighter if acid == at else acid
            if other not in reachable:
                reachable.add(other); queue.append(other)

    # Weighted least squares over the whole microstate graph, rather than a forward sweep averaging the
    ...
    for acid, lighter, value, weight in edges:
        if acid not in reachable or lighter not in reachable: continue
        a_i, b_i = slot.get(acid), slot.get(lighter)
        if a_i is not None:
            A[a_i][a_i] += weight; b[a_i] += weight * value
        if b_i is not None:
            A[b_i][b_i] += weight; b[b_i] -= weight * value
        if a_i is not None and b_i is not None:
            A[a_i][b_i] -= weight; A[b_i][a_i] -= weight
