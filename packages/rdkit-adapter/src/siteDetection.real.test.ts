 ...
 * from "I did not look".
 *
 * **Self-contained on purpose.** It reads `merged-labels.json` out of the vendor directory, which is the
 * same corpus the model trains on, so every number below regenerates from a bare checkout with
 * `SITE_DETECTION=1 pnpm vitest run siteDetection`. An earlier version read a truth file generated by an
 * ad-hoc script into a directory outside the repo, which made the published recall figure unreproducible —
 * exactly the failure `macro_validate.py` was written to end.
 *
