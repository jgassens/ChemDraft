...
describe("layout-engine page SVG planner", () => {
  ...
  it("exports marker connector paths before flattened arrowhead geometry", () => {
    const page = pageWithObjects([
      {
        id: "art_dashed_arrow",
        type: "graphic",
    ...
    const fragments = planPageSvgRender(page).fragments.flatMap(elementFragments);
    const path = fragments.find((fragment) => fragment.key === "graphic-path-art_dashed_arrow");
    const connector = fragments.find((fragment) => fragment.key === "graphic-marker-end-connector-art_dashed_arrow");
    const marker = fragments.find((fragment) => fragment.key === "graphic-marker-end-art_dashed_arrow");

    expect(path?.attrs).toMatchObject({
      class: "graphic-glyph-stroke graphic-glyph-path",
      "stroke-dasharray": "32 16"
    });
    expect(connector?.attrs).toMatchObject({
    ...
    expect(connector?.attrs["stroke-dasharray"]).toBeUndefined();
    expect(marker?.attrs).toMatchObject({
      "data-graphic-marker": "end",
      fill: "#12d64f",
      stroke: "none"
    });
    expect(fragments.indexOf(connector!)).toBeLessThan(fragments.indexOf(marker!));
  });
});
