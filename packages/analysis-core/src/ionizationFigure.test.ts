...
describe("what the figure draws", () => {
  it("puts the value on the structure, next to its own atom", () => {
    const svg = ionizationFigureSvg({
      structure: PHENOL,
      annotations: [{ atomIndex: 0, text: "9.95 ± 0.44", band: "good", tone: "acidic" }],
      title: "phenol"
    })!;
  ...
  it("draws the acidic hydrogen, because that is what the value is about", () => {
    ...
    const svg = ionizationFigureSvg({
      structure: PHENOL,
      annotations: [{ atomIndex: 0, text: "9.95", band: "good", tone: "acidic" }],
      title: "phenol"
    })!;
  ...
  it("shows a neutral amine as NH₂ rather than hiding its protons", () => {
    ...
    const svg = ionizationFigureSvg({
      structure: amine,
      annotations: [{ atomIndex: 0, text: "9.03 ± 4.76", band: "poor", tone: "acidic" }],
      title: "amine"
    })!;
  ...
  it("writes a charge as a superscript, not as a formula character", () => {
    ...
    const svg = ionizationFigureSvg({
      structure: nitro,
      annotations: [{ atomIndex: 0, text: "7.17", band: "good", tone: "acidic" }],
      title: "nitro"
    })!;
...
describe("nothing drawn is cut off", () => {
  it("frames the labels, not just the atoms", () => {
    ...
    const svg = ionizationFigureSvg({
      structure: PHENOL,
      annotations: [{ atomIndex: 0, text: "12.34 ± 56.78", band: "poor", tone: "acidic" }],
      title: "phenol"
    })!;
