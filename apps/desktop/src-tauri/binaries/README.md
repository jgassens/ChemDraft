...
depends only on KERNEL32.dll). The others are still placeholders; on Windows the
app checks for a PE image, so a placeholder reports the sidecar unavailable rather
